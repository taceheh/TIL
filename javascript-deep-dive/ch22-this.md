...
- **내가 이해한 문장**:
    - 다른 언어(Java, C++)와 달리 자바스크립트의 `this`는 함수가 **"어떻게 호출되었는지"** 에 따라 가리키는 대상이 그때그때 달라짐. (이 함수를 실행시킨 주체)
...
- **규칙**: 기본적으로 `this`는 전역 객체(**window** / global)에 바인딩
- **특이점**: `'use strict'`(엄격 모드)에서는 `undefined`가 됨
- **주의**: 중첩 함수나 콜백 함수라도 '일반 함수'로 호출되면 무조건 전역 객체를 가리킨다. (설계상의 결함으로 여겨짐)

### 3.2 메서드 호출 (Implicit Binding)
- **규칙**: 메서드를 호출한 객체, 즉 마침표(`.`) 앞의 객체에 바인딩
- **예시**: `person.getName()` -> `this`는 `person`임
- **핵심**: 메서드를 소유한 객체가 아니라, **호출한 객체**라는 점이 중요함.

### 3.3 생성자 함수 호출 (new Binding)
- **규칙**: 생성자 함수가 (미래에) 생성할 **인스턴스**에 바인딩.
- **동작**: `new` 키워드를 쓰면 빈 객체가 만들어지고, `this`는 그 빈 객체를 가리키게 됨.

### 3.4 간접 호출 (Explicit Binding)
- **규칙**: `Function.prototype.apply/call/bind` 메서드를 통해 `this`를 개발자가 **명시적으로 지정**

| 메서드 | 실행시점 | 인자 전달 방식 | 비유|
|---|---|---|---|
|**call**|**지금 즉시**|콤마(,)|"야 철수걸로 **지금** 해!"|
|**apply**|**지금 즉시**|배열([])|"야 영희 걸로 지금 하는데, 재료는 **봉투(배열)** 에 넣었어."|
|**bind**|**나중에**|(상관없음)|"실행은 안 할 건데, **주인 이름표만 미리 붙여서** 줘."|

[💾 예제 코드 실행해보기](./examples/22-02-this-binding-method.js)
...
- **정의**: 화살표 함수는 자체적인 `this` 바인딩을 갖지 않음
- **동작**: 상위 스코프(함수가 정의된 위치)의 `this`를 그대로 참조. (마치 변수 스코프 체인처럼 동작)
...
[💾 예제 코드 실행해보기](./examples/22-03-tricky-quiz.js)
