...
<br>
 ...
 [💾 예제 코드 실행해보기](./examples/24-01-lexical.js)
...
1. **참조의 연결과 콜 스택 제거** : 외부 함수(outer)가 종료되면 외부 함수의 실행 컨텍스트는 콜 스택에서 제거(Pop)됨
2. **렉시컬 환경의 독립적 존재** : 실행 컨텍스트가 제거되었다고 해서 그 안에 있던 렉시컬 환경(메모리 공간)까지 즉시 파괴되는 것은 아님
3. **가비지 컬렉터의 생존 규칙** : 자바스크립트의 가비지 컬렉터(Garbage Collector)는 다음의 원칙을 준수함(**"누군가가 참조하고 있는 메모리 공간은 절대 해제하지 않는다"**)
4. **렉시컬 환경의 생존 이유** : 내부 함수(inner)가 외부 함수의 렉시컬 환경을 자신의 [[Environment]]로 참조하고 있음. 이 내부 함수가 전역 변수 등에 의해 참조되고 있다면 외부 함수의 렉시컬 환경은 **"사용 중"** 인 상태로 간주됨. 따라서 힙(Heap) 메모리에 계속 살아있게 됨
...
[💾 예제 코드 실행해보기](./examples/24-03-loop-mistake.js)
...
1. **var의 스코프 문제** : var i는 블록 레벨 스코프가 아니라 함수 레벨 스코프를 준수. 따라서 for문의 코드 블록 { ... }은 스코프를 생성하지 않음. 즉, i는 전역 변수처럼 취급됨
2. **참조의 시점** : 배열 안에 담긴 3개의 함수는 모두 같은 전역 변수 i를 바라봄(참조)
3. **함수 실행 시점** : 함수가 실행(funcs[j]())되는 시점은 이미 for 루프가 완료되어 i가 3이 된 이후임
4. **최종 결과** : 3개의 함수 모두 "지금 i 값 내놔!"라고 하니, 전부 3을 가져옴
...
## 6. 헷갈렸던 부분 & 의문점
### 6.1 스코프와 클로저의 차이
- **Q.** 렉시컬 스코프랑 클로저랑 같은 말 아닌가?
- **A.** 렉시컬 스코프: 변수를 어디서 찾을지 결정하는 규칙(Rule)<br>클로저: 그 규칙을 이용해서 이미 사라진 환경을 기억하는 현상(Phenomenon) 혹은 그 함수 자체
엄밀히 다른 개념

### 6.2 가비지 컬렉터(GC) 동작 방식

- **Q.** 함수가 끝나면 내부 변수는 다 지워져야 하는 거 아닌가? 왜 살아있지?
- **A.** 가비지 컬렉터는 "누군가 참조하고 있는 메모리" 는 지우지 않음
내부 함수가 외부 함수의 변수를 잡고 있고, 그 내부 함수가 밖으로 반환되어 어딘가에서 쓰이고 있다면(참조된다면)
GC는 그 환경을 지우지 않음

### 6.3. 함수를 다시 호출하는 것(재귀)과 헷갈림
- **Q.** 내부 함수가 외부 함수를 잡고 있다는 게, `return outer()`처럼 외부 함수를 다시 호출한다는 뜻인가?
- **A.** **(X) 아니다.** 함수를 '호출(Call)'하는 동작이 아니라, 외부 함수의 **변수가 저장된 메모리 공간(렉시컬 환경)을 '참조(Reference)'**한다는 뜻이다.

**[비교]**
1. **재귀/호출 (X)**: `return outer()` -> 이건 그냥 함수를 다시 실행하는 것. 연결 고리가 아님.
2. **클로저/참조 (O)**: `console.log(outerVar)` -> 내부 함수가 외부 변수를 **사용(Use)**하는 순간, `[[Environment]]`를 통해 외부 환경과 물리적으로 연결됨.
...
function outer() {
  let a = 0;
  function inner() {
    return outer(); // 👈 이건 그냥 함수를 다시 '호출'하는 겁니다.
  }
}
```
...
function outer() {
  ...
  function inner() {
    console.log(a); // 2. 내부 함수가 외부 변수 a를 '사용'함! (이게 잡고 있는 것)
  }
  
  return inner; // 3. 내부 함수를 밖으로 내보냄
}

const myFunc = outer(); // outer는 실행 끝나서 죽었음
myFunc(); // 그런데 a값 10이 나옴! (inner가 잡고 있었으니까)
```
